...
On Windows and Linux the VoiceOver provider is present only as an unsupported-platform implementation. Linux advertises only `host.info`, `process.list`, and `process.info`; Windows additionally advertises the fixed NVDA recovery operations below.
