...
| INPUT-011 | Enforced in automated controller scope | A Hold Modifier is explicit owned state, never an accidental unbalanced chord. One action may own one to three non-overlapping held modifiers and optionally tap one additional key exactly once. From Base, its held modifiers remain down until the same action toggles them off. When armed from a physically held Action Layer, they are owned by that layer hold and release with it. Profile changes, remapping, inactivity/backgrounding, controller loss, or adapter teardown release held modifiers through their original route before local ownership is cleared. |
| INPUT-012 | Enforced in automated editor/profile scope | Every controller keyboard destination appears exactly once in one collapsed key category. Controller mapping and Quick Bar editors use the same selector. New profiles start with no mapped controller actions and no Quick Bar actions; creation immediately targets the new profile for editing rather than copying the recommended default layout. |
| INPUT-013 | Enforced in automated BSI/command scope; physical BSI validation required | Text Mode treats BSI Return as submit: queued live text is completed, Enter is sent once, Text Mode exits, and Quick Navigation returns. Quick Command preserves grammar-step boundaries and inserts only a fixed bounded settle delay between steps, so command → literal text → command sequences such as Win+R, executable name, Enter do not race UI creation. |
...
| ACCESSIBILITY-003 | Enforced in automated presentation-model scope; physical controller validation required | When a connected controller reports battery/capability information, Remote Control exposes it as ordinary accessible text. Missing battery data is reported as unavailable rather than guessed, and controller capability status never gates remote input. |
