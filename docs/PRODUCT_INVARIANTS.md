...
| INPUT-011 | Enforced in automated controller scope | A Hold Modifier is explicit owned state, never an accidental unbalanced chord. From Base it remains held across unrelated mapped key taps until toggled off. When armed from a physically held Action Layer, it is owned by that layer hold: ordinary inputs resolve through Base while other Hold Modifier actions in the layer remain reachable, and releasing the layer releases every modifier it owns. Profile changes, remapping, inactivity/backgrounding, controller loss, or adapter teardown release held modifiers through their original route before local ownership is cleared. |
