...
final class DualSenseControllerAdapter {
    ...
    private func configureTouchpad(for candidate: GCController) {
        ...
        if let touchpad = candidate.physicalInputProfile.touchpads.values.first {
            ...
            diagnostics.observe(source: .controller, result: "Touchpad: generic contact-state path installed")
        }
        ...
        if let dualSense = candidate.extendedGamepad as? GCDualSenseGamepad {
            ...
            primary.valueChangedHandler = { [weak self] _, x, _ in
                Task { @MainActor in self?.receiveDualSensePrimaryTouchpad(x: x) }
            }
            diagnostics.observe(source: .controller, result: "Touchpad: DualSense primary-finger path installed")
        }

        if touchpad == nil, dualSenseTouchpadPrimary == nil {
            diagnostics.observe(source: .controller, result: "Touchpad: no supported touch surface exposed by GameController")
        }
    }
    ...
    private func receiveDualSensePrimaryTouchpad(x: Float) {
        if !didObserveDualSensePrimaryMovement {
            ...
            diagnostics.observe(source: .controller, result: "Touchpad: DualSense primary-finger movement received")
        }
        if let touchpad {
            switch touchpad.touchState {
            case .down: receiveTouchpadContact(.down, x: x, source: "DualSense primary")
            case .moving: receiveTouchpadContact(.moving, x: x, source: "DualSense primary")
            case .up: receiveTouchpadContact(.up, x: x, source: "DualSense primary")
            @unknown default: resetTouchpadGesture()
            }
            return
        }
    ...
    private func receiveTouchpadContact(_ phase: ControllerTouchpadContactPhase, x: Float, source: String) {
        switch phase {
        case .down:
            ...
            if isActionLayerActiveForTouchpad { touchpadContactSuppressed = true }
        ...
        case .moving:
            if !touchpadContactActive {
                ...
                touchpadContactSuppressed = isActionLayerActiveForTouchpad
                touchpadRotor.begin(x: x)
                diagnostics.observe(source: .controller, result: "Touchpad: recovered contact from movement")
                return
            }
            if isActionLayerActiveForTouchpad { touchpadContactSuppressed = true }
            guard !touchpadContactSuppressed else { return }
    ...
    private var isActionLayerActiveForTouchpad: Bool {
        layerEngine.physicallyHeldLayerID != nil || layerEngine.state != .base
    }
    ...
    private func handleTouchpadMove(x: Float, source: String) {
        ...
        let change = direction > 0 ? quickNavigation.nextCategoryChange(in: rotorOrder) : quickNavigation.previousCategoryChange(in: rotorOrder)
        if quickNavigation.category == .profiles {
            quickNavigation.synchronizeProfileSelection(profiles: mappings.profiles, activeProfileID: mappings.activeProfileID)
        }
        diagnostics.observe(source: .controller, result: "Touchpad: rotor threshold crossed via \(source)")
        if settings.hapticFeedbackEnabled { controllerHaptics.play(change.wrapped ? .boundary : .selection) }
        announce(quickNavigation.currentSectionAnnouncement(quickBar: mappings.activeProfile.quickBar, profiles: mappings.profiles))
    }
    ...
    private func process(element: GCControllerElement, gamepad: GCExtendedGamepad) {
        ...
        handle(leftStick.update(x: gamepad.leftThumbstick.xAxis.value, y: gamepad.leftThumbstick.yAxis.value, at: now))
        handle(rightStick.update(x: gamepad.rightThumbstick.xAxis.value, y: gamepad.rightThumbstick.yAxis.value, at: now))
    }
        ...
        for phase in phases {
            ...
            case .pressed(let input):
                ...
                if started {
                    ...
                    } else if let stateChange = layerEngine.consumeOneShotAfterResolvedAction() {
                        present(stateChange)
                    }
                }
    ...
    private func start(action: ControllerAction, input: ControllerInput, eventID: Int) -> Bool {
        ...
        case .quickNavigation:
            ...
            if quickNavigation.isActive {
                quickNavigation.synchronizeProfileSelection(profiles: mappings.profiles, activeProfileID: mappings.activeProfileID)
                let section = quickNavigation.currentSectionAnnouncement(quickBar: mappings.activeProfile.quickBar, profiles: mappings.profiles)
    ...
    private func handleModeInput(_ input: ControllerInput, pressed: Bool, eventID: Int) -> Bool {
        if isQuickCommandModeActive {
            if input == .circle { setQuickCommandMode(false) }
            else { diagnostics.observeController(eventID: eventID, input: input, pressed: pressed, stage: "Quick Command Mode: controller input gated") }
            return true
        }
        if isTextModeActive {
            switch input {
            case .touchpadPress, .circle: setTextMode(false)
            case .rightStickPress: sendTextModeRemoteBackspace()
            default: diagnostics.observeController(eventID: eventID, input: input, pressed: pressed, stage: "Text Mode: controller input gated")
            }
            return true
        }
        if quickNavigation.isActive {
            ...
            case .rightStickUp:
                ...
                case .quickBar: announce(quickNavigation.previousQuickBarAction(in: mappings.activeProfile.quickBar))
                case .profiles: announce(quickNavigation.previousProfile(in: mappings.profiles))
                case .editing: announce(quickNavigation.previousEditingAction())
                default:
                    if let key = quickNavigation.category.key {
                        start(action: .keyboard(.init(key: key, modifiers: [.shift])), input: input, eventID: eventID)
                    }
                }
            case .rightStickDown:
                switch quickNavigation.category {
                case .quickBar: announce(quickNavigation.nextQuickBarAction(in: mappings.activeProfile.quickBar))
                case .profiles: announce(quickNavigation.nextProfile(in: mappings.profiles))
                case .editing: announce(quickNavigation.nextEditingAction())
                default:
                    if let key = quickNavigation.category.key { start(action: .keyboard(.init(key: key)), input: input, eventID: eventID) }
                }
            case .cross:
                switch quickNavigation.category {
                case .quickBar:
                    guard let entry = quickNavigation.selectedQuickBarEntry(in: mappings.activeProfile.quickBar), let action = entry.action else {
                        announce("Quick Bar action is unassigned")
                        return true
                    }
                    _ = executeQuickBarAction(action, input: input, eventID: eventID, recordAsLast: true)
                case .profiles:
                    guard let profile = quickNavigation.selectedProfile(in: mappings.profiles) else { announce("No profiles"); return true }
                ...
                case .editing:
                    start(action: .keyboard(quickNavigation.selectedEditingAction().keyboardAction), input: input, eventID: eventID)
                default:
    ...
    @discardableResult
    private func executeQuickBarAction(_ action: ControllerAction, input: ControllerInput, eventID: Int, recordAsLast: Bool) -> Bool {
        guard action.isAllowedInQuickBar else {
            diagnostics.observeController(eventID: eventID, input: input, pressed: true, stage: "Quick Bar: disallowed control action rejected")
            announce("That action is not available in Quick Bar")
            return false
        }
        let started = start(action: action, input: input, eventID: eventID)
        if started, recordAsLast, action.isRepeatableQuickBarAction { lastQuickBarAction = action }
        return started
    }

    @discardableResult
    private func performFarRelayAction(_ action: FarRelayControllerAction, input: ControllerInput, eventID: Int) -> Bool {
        switch action {
        case .textMode: setTextMode(!isTextModeActive); return true
        case .quickCommandMode: setQuickCommandMode(!isQuickCommandModeActive); return true
        case .repeatLastQuickBar:
            guard let lastQuickBarAction else { announce("No Quick Bar action to repeat"); return true }
            return executeQuickBarAction(lastQuickBarAction, input: input, eventID: eventID, recordAsLast: false)
        case .nextProfile:
    ...
    @discardableResult
    private func toggleStickyModifier(_ action: ControllerStickyModifierAction, input: ControllerInput, eventID: Int) -> Bool {
        guard action.isValid else {
            announce("Hold Modifier needs one to three modifiers")
            return false
        }

        let orderedModifiers = ControllerKeyboardModifier.allCases.filter { action.modifiers.contains($0) }
        let ownedLeases = orderedModifiers.compactMap { stickyModifierLeases[$0] }
        ...
        if ownedByThisInput,
           ...
           ownedLeases.allSatisfy({ $0.ownerLayerID == heldLayerID }) {
            ...
            if let tapKey = action.tapKey, let route = ownedLeases.first?.route {
                ...
                announce("\(held) remains held until \(heldLayerID.capitalized) layer is released")
            }
        ...
        if ownedByThisInput {
            let leases = orderedModifiers.compactMap { stickyModifierLeases.removeValue(forKey: $0) }
            routeStickyModifierLeases(leases, pressed: false, eventID: eventID, input: input)
        ...
        guard let targetID = router.activeTargetID,
            ...
            diagnostics.observeController(eventID: eventID, input: input, pressed: true, stage: "Sticky modifier: raw key target unavailable; no fallback")
            announce("Sticky modifier unavailable")
            return false
        }

        let resolved = orderedModifiers.map { ($0, stickyModifierVirtualKeys(for: $0)) }
        let physicalKeys = resolved.flatMap { $0.1 }
        guard !physicalKeys.isEmpty, Set(physicalKeys).count == physicalKeys.count else {
            announce("Those held modifiers overlap on this NVDA modifier configuration")
            return false
        }
        if let tapKey = action.tapKey, physicalKeys.contains(tapKey.virtualKey) {
        ...
        let leases = resolved.map { modifier, virtualKeys in
            StickyModifierLease(modifier: modifier, ownerInput: input, virtualKeys: virtualKeys, route: route, ownerLayerID: ownerLayerID)
        }
        for lease in leases { stickyModifierLeases[lease.modifier] = lease }

        routeStickyModifierActivation(leases, tapKey: action.tapKey, eventID: eventID, input: input)
    ...
    private func heldModifierDescription(_ modifiers: Set<ControllerKeyboardModifier>) -> String {
        ControllerKeyboardModifier.allCases.filter { modifiers.contains($0) }.map(\.label).joined(separator: " plus ")
    }

    private func routeStickyModifierActivation(_ leases: [StickyModifierLease], tapKey: WindowsKeyboardKey?, eventID: Int, input: ControllerInput) {
        guard let route = leases.first?.route else { return }
        diagnostics.observeController(eventID: eventID, input: input, pressed: true, stage: "Sticky modifier: \(leases.count) held modifier(s) activated")
        Task { @MainActor [router] in
            for lease in leases {
                for virtualKey in lease.virtualKeys {
                    _ = await router.route(.sendKeyTransition(.windowsVirtualKey(virtualKey), pressed: true), via: route)
                }
            }
            if let tapKey {
                let key = RemoteKey.windowsVirtualKey(tapKey.virtualKey)
    ...
    private func routeStickyModifierLeases(_ leases: [StickyModifierLease], pressed: Bool, eventID: Int, input: ControllerInput) {
        guard !leases.isEmpty else { return }
        diagnostics.observeController(eventID: eventID, input: input, pressed: pressed, stage: "Sticky modifier: \(leases.count) held modifier(s) \(pressed ? "held" : "released")")
        ...
        Task { @MainActor [router] in
            for lease in orderedLeases {
                ...
                for virtualKey in virtualKeys {
                    _ = await router.route(.sendKeyTransition(.windowsVirtualKey(virtualKey), pressed: pressed), via: lease.route)
                }
            }
        }
    }
    ...
    private func releaseStickyModifiers(ownedBy layerID: String) {
        let modifiers = ControllerKeyboardModifier.allCases.filter { stickyModifierLeases[$0]?.ownerLayerID == layerID }
        let leases = modifiers.compactMap { stickyModifierLeases.removeValue(forKey: $0) }
        routeStickyModifierLeases(leases, pressed: false, eventID: diagnosticEventID(), input: .home)
    }

    private func releaseStickyModifiers() {
        let leases = ControllerKeyboardModifier.allCases.compactMap { stickyModifierLeases.removeValue(forKey: $0) }
        routeStickyModifierLeases(leases, pressed: false, eventID: diagnosticEventID(), input: .home)
    }

    func suspendInputForInactiveContext() { releaseActiveActions() }
    ...
    private func setTextMode(_ active: Bool) {
        ...
        if active, isQuickCommandModeActive { setQuickCommandMode(false, restoreQuickNavigation: false, announceChange: false) }
        isTextModeActive = active
        textMirrorSession.reset()
        textModeBuffer = ""
        cancelTextOperations()
        if active { _ = quickNavigation.exit() }
        else { releaseActiveActions(); _ = quickNavigation.activate() }
    ...
    func submitTextModeAndExit() {
        ...
        textOperationTail = Task { @MainActor [weak self, previous] in
            ...
            guard let self, generation == self.textOperationGeneration, !Task.isCancelled, self.isTextModeActive else { return }
            await self.routeTextTap(.init(key: .enter), diagnostic: "Text Mode: submit Enter transmitted")
            guard generation == self.textOperationGeneration, !Task.isCancelled, self.isTextModeActive else { return }
            self.feedback.play(.success)
            self.setTextMode(false)
        }
    }

    private func setQuickCommandMode(_ active: Bool, restoreQuickNavigation: Bool = true, announceChange: Bool = true) {
        resetTouchpadGesture()
        if active, isTextModeActive { setTextMode(false) }
        ...
        cancelQuickCommand()
        if active { _ = quickNavigation.exit() }
        else if restoreQuickNavigation { releaseActiveActions(exitQuickNavigation: false); _ = quickNavigation.activate() }
        if announceChange { announce(active ? "Quick Command Mode" : "Quick Command Mode off") }
    }
    ...
    func prepareQuickCommandForConfirmation() -> String? {
        ...
        let command: QuickCommand
        do { command = try QuickCommandParser.parse(quickCommandBuffer) }
        ...
        do {
            switch target.kind {
            case .nvdaRemote: plan = try resolveWindowsQuickCommandPlan(command)
            case .macRemote: plan = try resolveMacQuickCommandPlan(command)
            default:
        ...
        preparedQuickCommand = PreparedQuickCommand(command: command, preview: preview, plan: plan, route: route)
        quickCommandStatus = "Ready to confirm."
        diagnostics.observe(source: .controller, result: "Quick Command: confirmation prepared; payload redacted")
        return preview
    }

    func cancelQuickCommandConfirmation() {
        preparedQuickCommand = nil
        if isQuickCommandModeActive, quickCommandStatus == "Ready to confirm." { quickCommandStatus = nil }
    }

    @discardableResult
    func confirmQuickCommand() -> Bool {
        guard isQuickCommandModeActive, let prepared = preparedQuickCommand else { return false }
        preparedQuickCommand = nil
        cancelQuickCommand()
        let generation = quickCommandGeneration
        diagnostics.observe(source: .controller, result: "Quick Command: confirmed \(prepared.command.steps.count) steps; payload redacted")
        quickCommandStatus = "Sending command."
        quickCommandTask = Task { @MainActor [weak self] in
            guard let self else { return }
            let result = await self.executeQuickCommandPlan(prepared.plan, via: prepared.route, generation: generation)
            ...
            case .performed:
                ...
                self.feedback.play(.success)
                let section = self.quickNavigation.currentSectionAnnouncement(quickBar: self.mappings.activeProfile.quickBar, profiles: self.mappings.profiles)
    ...
    func waitForQuickCommandForTesting() async { await quickCommandTask?.value }
    ...
    private enum QuickCommandExecutionError: Error {
        ...
        var message: String {
            switch self {
            case .unsupportedModifier(let name, let target): "\(name) is not available for \(target) Quick Command."
            case .unsupportedKey(let name, let target): "\(name) is not available for \(target) Quick Command."
            case .unsupportedTextCharacter(let target): "\(target) Quick Command text contains a character that cannot be typed yet."
            case .resolvedChordTooLarge: "Quick Command resolves to more than four physical keys at once."
            }
        }
    }
    ...
    private func resolveWindowsQuickCommandPlan(_ command: QuickCommand) throws -> [[[RemoteKey]]] {
        ...
        for step in command.steps {
            ...
            case .chord(let chord):
                ...
                guard virtualKeys.count <= QuickCommandParser.maximumChordKeys else { throw QuickCommandExecutionError.resolvedChordTooLarge }
                plan.append([virtualKeys.map(RemoteKey.windowsVirtualKey)])
            case .text(let text):
                var textStep: [[RemoteKey]] = []
                for character in text {
                    guard let action = ControllerTextCharacterMapper.action(for: character) else { throw QuickCommandExecutionError.unsupportedTextCharacter(target: "Windows/NVDA") }
                    let virtualKeys = windowsVirtualKeys(for: action.modifiers) + [action.key.virtualKey]
                    guard virtualKeys.count <= QuickCommandParser.maximumChordKeys else { throw QuickCommandExecutionError.resolvedChordTooLarge }
    ...
    private func resolveWindowsChord(_ chord: QuickCommandChord) throws -> [UInt16] {
        ...
        for key in chord.keys {
            ...
            case .modifier(let modifier):
                ...
                case .nvda:
                    switch settings.nvdaModifier { case .capsLock: result.append(VK.capital); case .voKeys: result.append(contentsOf: [VK.control, VK.menu]) }
                case .command: throw QuickCommandExecutionError.unsupportedModifier("Command", target: "Windows/NVDA")
                case .option: throw QuickCommandExecutionError.unsupportedModifier("Option", target: "Windows/NVDA")
                }
            case .key(let key): result.append(try windowsVirtualKey(for: key))
            }
        }
        guard Set(result).count == result.count else { throw QuickCommandExecutionError.unsupportedKey("Duplicate resolved key", target: "Windows/NVDA") }
        return result
    }

    private func windowsVirtualKeys(for modifiers: Set<ControllerKeyboardModifier>) -> [UInt16] {
        let order: [ControllerKeyboardModifier] = [.control, .alt, .shift, .windows, .nvda]
        var result: [UInt16] = []
        for modifier in order where modifiers.contains(modifier) { result.append(contentsOf: stickyModifierVirtualKeys(for: modifier)) }
        return result
    }
    ...
    private func windowsVirtualKey(for key: QuickCommandKey) throws -> UInt16 {
        ...
        case .function(let number):
            guard (1...24).contains(number) else { throw QuickCommandExecutionError.unsupportedKey("Function key", target: "Windows/NVDA") }
        ...
        case .character(let character):
            ...
            if let scalar = lower.unicodeScalars.first, lower.unicodeScalars.count == 1 {
                if (97...122).contains(scalar.value) { return UInt16(scalar.value - 32) }
                if (48...57).contains(scalar.value) { return UInt16(scalar.value) }
            }
            throw QuickCommandExecutionError.unsupportedKey(String(character), target: "Windows/NVDA")
    ...
    private func resolveMacQuickCommandPlan(_ command: QuickCommand) throws -> [[[RemoteKey]]] {
        ...
        for step in command.steps {
            ...
            case .chord(let chord):
                ...
                guard usages.count <= QuickCommandParser.maximumChordKeys else { throw QuickCommandExecutionError.resolvedChordTooLarge }
                plan.append([try usages.map { usage in
                    guard MacRemoteKey.supportedKeyboardUsage(usage) != nil, let key = RemoteKey.hidUsage(usage) else { throw QuickCommandExecutionError.unsupportedKey("Key", target: "Mac") }
                    return key
                }])
            case .text(let text):
                var textStep: [[RemoteKey]] = []
                for character in text {
                    guard let usages = macHIDChord(for: character) else { throw QuickCommandExecutionError.unsupportedTextCharacter(target: "Mac") }
                    textStep.append(try usages.map { usage in
                        guard MacRemoteKey.supportedKeyboardUsage(usage) != nil, let key = RemoteKey.hidUsage(usage) else { throw QuickCommandExecutionError.unsupportedKey("Key", target: "Mac") }
                        return key
                    })
                }
    ...
    private func resolveMacChord(_ chord: QuickCommandChord) throws -> [UInt16] {
        ...
        for key in chord.keys {
            ...
            case .modifier(let modifier):
                ...
                case .windows: throw QuickCommandExecutionError.unsupportedModifier("Windows", target: "Mac")
                case .nvda: throw QuickCommandExecutionError.unsupportedModifier("NVDA", target: "Mac")
                }
            case .key(let key): result.append(try macHIDUsage(for: key))
            }
        }
        guard Set(result).count == result.count else { throw QuickCommandExecutionError.unsupportedKey("Duplicate resolved key", target: "Mac") }
        return result
    }
    ...
    private func macHIDUsage(for key: QuickCommandKey) throws -> UInt16 {
        switch key {
        case .function(let number):
            guard (1...12).contains(number) else { throw QuickCommandExecutionError.unsupportedKey("F\(number)", target: "Mac") }
            return 0x3A + UInt16(number - 1)
        case .character(let character):
            let value = String(character).lowercased()
            guard value.unicodeScalars.count == 1, let scalar = value.unicodeScalars.first else { throw QuickCommandExecutionError.unsupportedKey(String(character), target: "Mac") }
            if (97...122).contains(scalar.value) { return 0x04 + UInt16(scalar.value - 97) }
            if (49...57).contains(scalar.value) { return 0x1E + UInt16(scalar.value - 49) }
            if scalar.value == 48 { return 0x27 }
            throw QuickCommandExecutionError.unsupportedKey(String(character), target: "Mac")
        case .named(let named):
            let usage: UInt16? = switch named {
            case .tab: 0x2B; case .enter: 0x28; case .escape: 0x29; case .space: 0x2C; case .backspace: 0x2A; case .delete: 0x4C; case .insert: 0x49; case .home: 0x4A; case .end: 0x4D; case .pageUp: 0x4B; case .pageDown: 0x4E; case .left: 0x50; case .right: 0x4F; case .up: 0x52; case .down: 0x51; case .capsLock: 0x39; case .comma: 0x36; case .period: 0x37; case .slash: 0x38; case .backslash: 0x31; case .semicolon: 0x33; case .quote: 0x34; case .grave: 0x35; case .minus: 0x2D; case .equal: 0x2E; case .leftBracket: 0x2F; case .rightBracket: 0x30; case .pause, .printScreen, .scrollLock, .numLock, .contextMenu: nil
            }
            guard let usage, MacRemoteKey.supportedKeyboardUsage(usage) != nil else { throw QuickCommandExecutionError.unsupportedKey(named.spokenName, target: "Mac") }
            return usage
        }
    }
    ...
    private func macHIDChord(for character: Character) -> [UInt16]? {
        ...
        switch scalar.value {
        case 65...90: return shifted(0x04 + UInt16(scalar.value - 65)); case 97...122: return [0x04 + UInt16(scalar.value - 97)]; case 49...57: return [0x1E + UInt16(scalar.value - 49)]; case 48: return [0x27]; case 9: return [0x2B]; case 10, 13: return [0x28]; case 32: return [0x2C]; case 33: return shifted(0x1E); case 34: return shifted(0x34); case 35: return shifted(0x20); case 36: return shifted(0x21); case 37: return shifted(0x22); case 38: return shifted(0x24); case 39: return [0x34]; case 40: return shifted(0x26); case 41: return shifted(0x27); case 42: return shifted(0x25); case 43: return shifted(0x2E); case 44: return [0x36]; case 45: return [0x2D]; case 46: return [0x37]; case 47: return [0x38]; case 58: return shifted(0x33); case 59: return [0x33]; case 60: return shifted(0x36); case 61: return [0x2E]; case 62: return shifted(0x37); case 63: return shifted(0x38); case 64: return shifted(0x1F); case 91: return [0x2F]; case 92: return [0x31]; case 93: return [0x30]; case 94: return shifted(0x23); case 95: return shifted(0x2D); case 96: return [0x35]; case 123: return shifted(0x2F); case 124: return shifted(0x31); case 125: return shifted(0x30); case 126: return shifted(0x35); default: return nil
        }
    }

    private func executeQuickCommandPlan(_ plan: [[[RemoteKey]]], via route: RemoteIntentRoute, generation: Int) async -> RemoteIntentResult {
        for (stepIndex, step) in plan.enumerated() {
            for chord in step {
                guard generation == quickCommandGeneration, !Task.isCancelled, isQuickCommandModeActive else { return .unavailable("Quick Command was cancelled.") }
                var pressed: [RemoteKey] = []
                for key in chord {
                    guard generation == quickCommandGeneration, !Task.isCancelled, isQuickCommandModeActive else { await releaseQuickCommandKeys(pressed, via: route); return .unavailable("Quick Command was cancelled.") }
                    pressed.append(key)
                    let result = await router.route(.sendKeyTransition(key, pressed: true), via: route)
                    if result != .performed { await releaseQuickCommandKeys(pressed, via: route); return result }
                }
                for key in pressed.reversed() {
                    let result = await router.route(.sendKeyTransition(key, pressed: false), via: route)
                    if result != .performed { return result }
                }
            }
            if stepIndex < plan.count - 1 {
                do { try await Task.sleep(nanoseconds: QuickCommandExecutionPolicy.interStepDelayNanoseconds) }
                catch { return .unavailable("Quick Command was cancelled.") }
            }
        }
        return .performed
    }

    private func releaseQuickCommandKeys(_ keys: [RemoteKey], via route: RemoteIntentRoute) async {
        for key in keys.reversed() { _ = await router.route(.sendKeyTransition(key, pressed: false), via: route) }
    }

    private func announcePrivate(_ text: String) { UIAccessibility.post(notification: .announcement, argument: text) }
    ...
    func applyTextModeEditorValue(_ proposedValue: String) -> String {
        ...
        if proposed.starts(with: old) { mirrorTextInsertion(Array(proposed.dropFirst(old.count))) }
        else if old.starts(with: proposed) { mirrorTextBackspace(count: old.count - proposed.count) }
        else { announce("Text Mode supports appending and deleting from the end") }
    ...
    func mirrorTextInsertion(_ characters: [Character]) {
        ...
        for character in characters {
            ...
            } else { textMirrorSession.append(character, mirrored: false); unsupported += 1 }
        }
        textModeBuffer = textMirrorSession.text
        if unsupported > 0 { announce("Unsupported character"); diagnostics.observe(source: .controller, result: "Text Mode: \(unsupported) unsupported character") }
    }
    ...
    func mirrorTextBackspace(count: Int = 1) {
        ...
        textModeBuffer = textMirrorSession.text
        for _ in 0..<remoteBackspaces { enqueueTextTap(.init(key: .backspace), diagnostic: "Text Mode: remote backspace transmitted") }
    }
    ...
    private func routeTextTap(_ action: KeyboardAction, diagnostic: String) async {
        guard let targetID = router.activeTargetID else { diagnostics.observe(source: .controller, result: "\(diagnostic); no active target"); return }
    ...
    private func present(_ stateChange: LayerFeedback) {
        switch stateChange {
        case .activated, .oneShot: feedback.play(.selectionAccepted)
        case .locked: feedback.play(.success)
        case .base: feedback.play(.layerExit, haptic: .selectionAccepted)
        }
    ...
    private func route(_ active: ActiveAction, transition: ActionTransition) {
        ...
        let stage = switch transition { case .pressed: "press"; case .repeated: "repeat"; case .released: "release" }
        let pressed = stage != "release"
        diagnostics.observeController(eventID: active.eventID, input: active.input, pressed: pressed, stage: "RemoteIntent created: \(stage) keyboard virtual key \(action.key.virtualKey)")
        ...
        Task { @MainActor [router, diagnostics, route = active.route, input = active.input, eventID = active.eventID] in
            ...
            case .performed: "Transport: queued; host receipt/execution: unconfirmed by the existing IPC protocol"
            case .unsupported: "Capability routing: executor reported unsupported; no fallback"
            case .unavailable(let reason): "Executor unavailable: \(reason)"
            case .failed(let reason): "Transport/execution failed: \(reason)"
            }
    ...
    func beginTouchpadSwipeForTesting(x: Float) { receiveTouchpadContact(.down, x: x, source: "test") }
    func moveTouchpadForTesting(x: Float) { guard touchpadContactActive else { return }; receiveTouchpadContact(.moving, x: x, source: "test") }
    func endTouchpadSwipeForTesting() { receiveTouchpadContact(.up, x: 0, source: "test") }
    func receiveTouchpadContactForTesting(_ phase: ControllerTouchpadContactPhase, x: Float) { receiveTouchpadContact(phase, x: x, source: "test") }
    func receiveDpadForTesting(up: Bool, down: Bool, left: Bool, right: Bool, at time: TimeInterval) { receiveDpadState(up: up, down: down, left: left, right: right, at: time) }
    func waitForTextOperationsForTesting() async { await textOperationTail?.value }

    private func receiveDpadState(up: Bool, down: Bool, left: Bool, right: Bool, at time: TimeInterval) {
        let states: [(ControllerInput, Bool)] = [(.dpadUp, up), (.dpadDown, down), (.dpadLeft, left), (.dpadRight, right)]
        for (input, pressed) in states { handle(inputLifecycle.receive(input, pressed: pressed, at: time)) }
    }

    private func diagnosticEventID() -> Int { defer { nextDiagnosticEventID += 1 }; return nextDiagnosticEventID }
    ...
    private func releaseActiveActions(exitQuickNavigation: Bool = true) {
        ...
        let actions = ControllerInput.allCases.compactMap { input in activeActions[input].map { (input, $0) } }
    ...
    private func resolvedModifiers(_ modifiers: Set<ControllerKeyboardModifier>) -> Set<RemoteModifier> {
        modifiers.reduce(into: []) { result, modifier in
            ...
            case .nvda:
                switch settings.nvdaModifier { case .capsLock: result.insert(.capsLock); case .voKeys: result.formUnion([.control, .alt]) }
            }
        }
    }
